...
>(
	(
		...
		return (
			<div
				ref={ref}
				className="group peer z-50 hidden text-sidebar-foreground md:block"
				data-state={state}
		...
		return (
			<>
				<div
					...
					data-state={state}
					className={cn(
						"relative flex h-full flex-col gap-4 overflow-hidden bg-sidebar p-4 transition-[width] duration-300 ease-in-out",
						{
							"w-0": !open && !isMobile,
							[`w-[${SIDEBAR_WIDTH}]`]: open && !isMobile,
							[`w-[${SIDEBAR_WIDTH_ICON}]`]:
								collapsible === "icon" && !open && !isMobile,
						},
						className
					)}
					style={
				...
				<Sheet open={openMobile} modal>
					<SheetContent
						side="left"
						className="flex flex-col gap-4 overflow-y-auto bg-sidebar p-4"
					>
						{children}
					</SheetContent>
				</Sheet>
